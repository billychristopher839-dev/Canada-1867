...
(function () {
  ...
  function playTrack(i) {
    trackIdx = ((i % D.tracks.length) + D.tracks.length) % D.tracks.length;
    audio.src = D.tracks[trackIdx].src;
    const p = audio.play();
    if (p && p.catch) p.catch(() => { /* autoplay blocked until user gesture */ });
    nowPlaying.textContent = "Now playing: " + D.tracks[trackIdx].title;
    nowPlaying.classList.remove("hidden");
  }
  function setMusic(on) {
    musicOn = on;
    musicBtn.setAttribute("aria-pressed", String(on));
    musicLabel.textContent = on ? "Music on" : "Play music";
    if (on) {
      if (!audio.src) playTrack(trackIdx); else audio.play().catch(() => {});
      nowPlaying.classList.remove("hidden");
    } else {
      audio.pause();
  ...
  audio.addEventListener("ended", () => playTrack(trackIdx + 1)); // loops playlist in the same order
